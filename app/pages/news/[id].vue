<template>
  <div class="mx-auto md:w-[85%] px-4 md:py-12 py-10 mb-8" dir="rtl">
    <NuxtLink
      to="/news"
      class="inline-flex items-center gap-2 mb-8 mx-4 md:mb-2 text-lg font-medium"
    >
      العودة إلى الأخبار
      <span class="text-xl">←</span>
    </NuxtLink>

    <div v-if="!newsItem" class="text-center py-20 text-xl">
      الخبر غير موجود (المعرف: {{ route.params.id }})
    </div>

    <article v-if="newsItem" class="rounded-2xl px-6 overflow-hidden">
      <div class=" ">
        <time class="block text-base md:text-lg my-4">
          {{ newsItem.date }}
        </time>
        <h1
          class="text-2xl md:text-3xl lg:text-4xl font-bold mb-5 leading-tight"
        >
          {{ newsItem.title }}
        </h1>
      </div>
      <div class="py-4 mb-5 md:mb-2">
        <img
          v-if="newsItem.image"
          :src="newsItem.image"
          :alt="newsItem.title"
          class="w-full md:h-screen h-64 rounded-2xl object-cover"
          loading="lazy"
        />
      </div>
      <p v-if="newsItem.summary" class="text-lg md:text-xl mb-8">
        {{ newsItem.summary }}
      </p>
      <div class="flex items-center gap-2">
        <h4 class="text-lg md:text-xl">شارك هذه المقالة</h4>
        <ul class="space-y-3">
          <li class="flex gap-2">
            <a
              href="javascript:;"
